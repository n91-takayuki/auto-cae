...
_STRATEGIES: list[_Strategy] = [
    ...
    # 7. NEW: Shift algorithms to robust MeshAdapt/Delaunay and DOUBLE the global mesh size.
    _Strategy("Coarse Delaunay tet4",        1,  1, 1, False, False, True, 2.0),
    # 8. NEW: The Nuclear Option. QUADRUPLE the mesh size. Swallows almost any CAD errors.
...
def _setup_strategy(gmsh, strat: _Strategy, target: float) -> None:
    ...
    # Apply heavy OCC healing if requested. This fixes topological disconnects.
    if getattr(strat, "heal_occ", False):
